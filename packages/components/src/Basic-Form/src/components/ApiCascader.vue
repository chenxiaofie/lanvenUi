...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    components: {
      LoadingOutlined,
      [Cascader.name as string]: Cascader,
    },
    props: {
      value: {
        type: Array,
      ...
      isShowParent: propTypes.bool.def(true),
      // init fetch params
      initFetchParams: {
    ...
    setup(props, { emit }) {
      ...
      function handleRenderDisplay({ labels, selectedOptions }: any) {
        if (unref(emitData).length === selectedOptions.length) {
          if (props.isShowParent) {
            return labels.join(' / ');
          } else {
            return labels[selectedOptions.length - 1];
          }
        }
        if (props.displayRenderArray != null) {
          // if (props.isShowParent) {
          //   return props.displayRenderArray.join(' / ');
          // } else {
          //   return props.displayRenderArray[props.displayRenderArray.length - 1];
          // }
          const res = props.displayRenderArray(props);
          return res;
        } else {
          return props.value ? props.value[props.value.length - 1] : '';
        }
      }
