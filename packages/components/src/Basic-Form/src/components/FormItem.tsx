...
import type { TableActionType } from '@/src/Basic-Table';
import { Col, Divider, Form } from 'ant-design-vue';
import { componentMap } from '../componentMap';
import { BasicHelp } from '@/helpComponents/Basic';
...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    function renderComponent() {
      ...
      const isSelect =
        component && ['ApiSelect', 'Select', 'Switch', 'Checkbox'].includes(component);

      const eventKey = `on${upperFirst(changeEvent)}`;

      const on = {
        [eventKey]: (...args: Nullable<Recordable>[]) => {
          const [e, content] = args;
          ...
          if (isSelect) {
            emit(changeEvent, { field, content });
          }
        },
      };
      ...
      if (isCreatePlaceholder && component !== 'RangePicker' && component) {
        propsData.placeholder =
          unref(getComponentsProps)?.placeholder || createPlaceholderMessage(component, placehold);
      }
      ...
      return (
        isIfShow && (
          <Col {...realColProps} v-show={isShow}>
            <ConfigProvider locale={locale}>{getContent()}</ConfigProvider>
          </Col>
        )
      );
    };
